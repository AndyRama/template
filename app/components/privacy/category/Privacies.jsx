...
const Items = ({ currentItems, Privacies }) => {

  return (
    <>
      { currentItems &&
        currentItems.map((privacy, index) => {
          index *= 0.05
          return (
            <motion.div
              ...
              key={privacy.title}>
              <Link href={`/${privacy.url}`} className='relative block overflow-hidden'>
                <Image
                  src={privacy.image}
                  alt={privacy.title}
                  width={1064}
...
const Privacies = ({ className, itemsPerPage, archive= false, params }) => {
  ...
  if(archive === false) {
    items = allPrivacies.sort((a,b) =>
      compareDesc(new Date(a.date), new Date(b.date))
    )
  } else {
    if(params?.slug) {
      items = allPrivacies.filter((privacy) =>
        privacy.categories.some(
          (category) =>
            category.title
            .toLowerCase()
            .trim()
